...
<head>
<meta charset="UTF-8">
<title>/jquery/Step06_example.jsp</title>
</head>
<body>
<div class="container">
	<h1>회원가입 폼입니다.</h1>
...
<script>
		...
		$.ajax({
			...
			success:function(responseData){
				...
				if(responseData.canUse){//사용가능한 아이디
					...
					$("#feedbackId").text("사용 불가능한 아이디 입니다.").css("color","red");
					isIdValid=false;
				}
			}
		})
	...
	$("#pwd").on("input",function(){
		...
		if(inputPwd.length>=5){
			...
			isPwdValid=false;
			$("#feedbackPwd").text("5글자 이상 입력하세요.").css("color","red");
		}
	});
	
	//폼에 submit 이벤트가 일어났을때 실행할 함수 등록
	$("#signupForm").on("submit",function(){
		//만일 아이디나 비밀번호가 유효하지 않으면 폼 제출을 막아야한다(return false).
